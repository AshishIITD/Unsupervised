...
def generate_html_word_file():
    output_path = "/home/gaian/Unsupervised/Unsupervised_Learning_Walkthroughs.html"
    
    html_content = r"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Unsupervised Learning Masterclass: Numerical Walkthroughs</title>
    <style>
        body {
...
<body>
    ...
    <h1>Unsupervised Learning Masterclass:<br>Numerical Walkthroughs</h1>
    
    <p style="font-size: 1.15rem; text-align: center; font-style: italic; color: #4a5568; margin-bottom: 50px;">
        A hand-calculable pedagogical guide tracing the core pillars of unsupervised learning.
    </p>

    <!-- SECTION 1: K-MEANS -->
    <h2>1. K-Means Clustering: Step-by-Step Walkthrough</h2>
    <p>
    ...
    <!-- SECTION 2: DBSCAN -->
    <h2>2. DBSCAN Clustering: Step-by-Step Walkthrough</h2>
    <p>
...
● [X] ● 
   &epsilon;  ○   (Border point: connected to Core point X, but not dense itself)</pre>
        ...
        <li>
            <strong>3. Noise Point:</strong> A point with too few neighbors and not reachable from any core point.
            <pre style="font-family: monospace; background: #f7fafc; padding: 6px 12px; margin: 5px 0; width: fit-content; line-height: 1.2;">
●  ●  ●
...
algorithm ExpandCluster(dataset, point, neighbors, cluster_id, eps, min_pts) is
    ...
    <!-- SECTION 3: PCA -->
    <h2>3. PCA Dimensionality Reduction: Step-by-Step Walkthrough</h2>
    <p>
    ...
    <ol>
        <li>Calculate averages: Mean X = $\frac{1+3+5}{3} = 3$, Mean Y = $\frac{2+4+6}{3} = 4$.</li>
        <li>Subtract the means:
            <ul>
                <li><strong>Point A'</strong> = $(1 - 3, 2 - 4) = \mathbf{(-2, -2)}$</li>
                <li><strong>Point B'</strong> = $(3 - 3, 4 - 4) = \mathbf{(0, 0)}$</li>
                <li><strong>Point C'</strong> = $(5 - 3, 6 - 4) = \mathbf{(2, 2)}$</li>
            </ul>
        </li>
    </ol>
    ...
    <p><strong>Step 3: Find Eigenvalues & Eigenvectors</strong><br>
       1. Solve $\det(S - \lambda I) = 0$:
          $$\det \begin{pmatrix} 4 - \lambda & 4 \\ 4 & 4 - \lambda \end{pmatrix} = 0 \implies \lambda^2 - 8\lambda = 0$$
       ...
       2. Solve for the eigenvector $\mathbf{v}$ for $\lambda_1 = 8$:
          $$\begin{pmatrix} -4 & 4 \\ 4 & -4 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix} = 0 \implies x = y$$
    ...
    <h2>4. Isolation Forest Outliers: Step-by-Step Walkthrough</h2>
    <p>
    ...
    <!-- SECTION 5: APRIORI -->
    <h2>5. Apriori Association Rules: Step-by-Step Walkthrough</h2>
    <p>
    ...
    <ul>
        <li><strong>{Bread}</strong>: 2 receipts (T1, T2) &rarr; Support = $\frac{2}{3} = \mathbf{66.7\%}$ (Passes!)</li>
        <li><strong>{Butter}</strong>: 2 receipts (T1, T2) &rarr; Support = $\frac{2}{3} = \mathbf{66.7\%}$ (Passes!)</li>
        <li><strong>{Milk}</strong>: 2 receipts (T2, T3) &rarr; Support = $\frac{2}{3} = \mathbf{66.7\%}$ (Passes!)</li>
    </ul>
    ...
    <ul>
        <li><strong>{Bread, Butter}</strong>: 2 receipts (T1, T2) &rarr; Support = $\frac{2}{3} = \mathbf{66.7\%}$ (Passes!)</li>
        <li><strong>{Bread, Milk}</strong>: 1 receipt (T2) &rarr; Support = $\frac{1}{3} = \mathbf{33.3\%}$ (<span class="highlight">❌ Pruned! Below 50%</span>)</li>
        <li><strong>{Butter, Milk}</strong>: 1 receipt (T2) &rarr; Support = $\frac{1}{3} = \mathbf{33.3\%}$ (<span class="highlight">❌ Pruned! Below 50%</span>)</li>
    </ul>
    ...
    <ul>
        <li><strong>Rule A: $[Bread] \rightarrow [Butter]$</strong><br>
            ...
            - Confidence (Reliability): $\frac{\text{Support}(Bread, Butter)}{\text{Support}(Bread)} = \frac{2}{2} = \mathbf{100\%}$ (Passes &ge; 60%!)<br>
            - Lift (Strength): $\frac{\text{Confidence}}{\text{Support}(Butter)} = \frac{1.00}{0.667} = \mathbf{1.5x}$ (Positive correlation!)
        </li>
        <li style="margin-top: 15px;"><strong>Rule B: $[Butter] \rightarrow [Bread]$</strong><br>
            - Support: $\mathbf{66.7\%}$<br>
            - Confidence: $\frac{\text{Support}(Bread, Butter)}{\text{Support}(Butter)} = \frac{2}{2} = \mathbf{100\%}$ (Passes!)<br>
...
"""
    ...
    print(f"[+] Beautiful Word-Compatible HTML document generated at: {output_path}")

if __name__ == "__main__":
    generate_html_word_file()
